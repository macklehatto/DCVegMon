...
```{r}
#filtering just 2023 data

#Filter for data of interest

dcveg2021 <- AllData %>% filter(AllData[,3] == "LHTS2021" )

#grabbing other data of interest or reference data
...
LHTSanalysis <- LHTSandref %>%
  mutate(midpoint_cc = ifelse(is.na(midpoint_cc), 0, midpoint_cc)) %>%
  pivot_wider(names_from = scientific_name, values_from = midpoint_cc, values_fn = list)

#produce ordination plot of filtered data and reference sites

LHTSanalysis[,-1] <- sapply(LHTSanalysis[,-1], function(x) as.numeric(as.character(x)))

LHTSanalysis[is.na(LHTSanalysis)] <- 0

LHTSanalysis <- column_to_rownames(LHTSanalysis, var="releveCode")

ord <- decorana(LHTSanalysis[,-1])
