...
## Change this to the filepath for the data directory for releve data from 2020 and 2021
...
old <- "C:\\Users\\Meg\\OneDrive - University of Nebraska-Lincoln\\R practice\\DCVegMon\\data\\2019 and prior data\\Compiled Releve Data for R.xlsx"
...
```{r}
#  test <- read_xlsx("C:\\Users\\Meg\\OneDrive - University of Nebraska-Lincoln\\R practice\\DCVegMon\\data\\GRG Reference\\List-3-Central-Mesic-Hardwood-Forest-Eastern.xlsx", skip = 2)
# 
#  test$row_num <- seq.int(nrow(test))
# > subcanopy <- which(grepl("Understory Trees", test$...2))
# > subcanopy
# integer(0)
# > subcanopy <- which(grepl("Understory Trees", test$`Canopy Trees (>10 m)`))
# > subcanopy
# [1] 14
# > canopy <- test %>%
# +     filter(row_num < subcanopy) %>%
# +     select(-row_num)
# Error in select(., -row_num) : unused argument (-row_num)
# > 
# > canopy <- test %>%
# +     filter(row_num < subcanopy) %>%
...
* -Create matrix
* -Groups Table - what is this and how do we automate import of new data?
