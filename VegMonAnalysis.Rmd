...
##THIS CAN BE SPLIT OUT TO RUN ONLY ONCE - AT THE END ADD CODE TO SAVE IT AS .CSV IN A SPECIFIC DATA FOLDER FOR PROCESSED DATA --DONE on 12/9. New script file is called "Process this years datasheets.R"
...
old <- here("Data", "2019 and prior data", "Compiled Releve Data for R.xlsx")
  ...
  olddata <- read_excel(old, sheet = "Sheet1", col_name = TRUE) %>%
    dplyr::select("Releve", "Species", "Midpoint % Cover") %>%
    janitor::clean_names() %>%
...
{ 
  refdata <- read_xlsx(y, sheet= 1, skip = 1, col_names = TRUE) %>%
    janitor::remove_empty(c("rows"))%>%
    dplyr::select("Genus", "Species", "3Freq" , "4Abund", "5Index")
...
# 
## ON 12/9/25 CHRIS AND MEG DIDN'T REALLY REMEMBER DOING THIS. KEEP FOR FUTURE INQUIRY
...
## THIS IS THE START OF THE ORDINATION

####This is the old ordination code
...
### This is the new workflow code (12/10/25)

FilesPreppedForOrd <- list.files(path = "data/Data Prepared for Ordination", pattern = "\\.csv$", full.names = TRUE)

DataframesForOrd <- lapply(FilesPreppedForOrd, read.csv)
...
dcveg <- AllData %>%
  mutate(releveCode = paste(releveSite, releveyear, sep = "")) %>%
  dplyr::select(-releveSite) %>%
  dplyr::select(-releveyear) %>%
...
dcveg[,-2] <- sapply(dcveg[,-2], function(x) as.numeric(as.character(x)))

dcveg[is.na(dcveg)] <- 0

dcveg <- column_to_rownames(dcveg, var="releveCode")

ord <- decorana(dcveg[,-2])

shnam <- make.cepnames(names(dcveg))
...
sel2 <- orditorp(ord, display = "sites", type = 't', pcol = "gray", pch = "+")

sel <- orditorp(ord, dis = "sites", type = "n", col = "blue")
text(ord, display = "sites", cex = 0.7, col = "purple")
...
dcveg2025 <- AllData %>% filter(grepl('2025', releveCode))
...
OrdAnalysis <- DataOrd2025 %>%
  mutate(midpoint_cc = ifelse(is.na(midpoint_cc), 0, midpoint_cc)) %>%
  pivot_wider(names_from = scientific_name, values_from = midpoint_cc, values_fn = list)

##produce ordination plot of filtered data and reference sites

OrdAnalysis[,-1] <- sapply(OrdAnalysis[,-1], function(x) as.numeric(as.character(x)))

OrdAnalysis[is.na(OrdAnalysis)] <- 0

OrdAnalysis <- column_to_rownames(OrdAnalysis, var="releveCode")

ord25 <- decorana(OrdAnalysis[,-1])
